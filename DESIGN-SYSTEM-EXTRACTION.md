...
Look for nodes of type `COMPONENT_SET`. Record the name and ID:
```json
{
  "buttons": {
    "componentSets": [
      { "name": "Button", "id": "57994:2227" },
      { "name": "FAB", "id": "57998:43426" },
      { "name": "Icon button", "id": "58000:1199" }
    ]
  }
}
```
...
Record the axes and their values:
```json
{
  "name": "Button",
  "id": "57994:2227",
...
### Rule 4: Verify IDs are current

Component node IDs are specific to the Figma file. If the design kit is updated or duplicated, IDs will change. Before generating screens with a previously extracted JSON:
...
### Rule 5: Extract only what you need
...
### Rule 6: Record the font fallback
